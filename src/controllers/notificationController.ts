...
import notificationService, { NotificationServiceError } from '../services/notificationService.js';
...
export const getNotifications = async (req: ArtistRequest, res: Response) => {
  try {
    ...
    const { rows, count } = await notificationService.getNotificationsByArtist(artistId, limit, offset);
...
export const markAsRead = async (req: ArtistRequest, res: Response) => {
  try {
    ...
    const notification = await notificationService.markAsRead(notificationId, artistId);
